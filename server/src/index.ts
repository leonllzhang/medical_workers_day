...
const DEFAULT_TEAMS: Team[] = [
  { id: 'team-1',  name: '内科',  buzzerNumber: 1,  score: 0, color: '#4d96ff' },
  { id: 'team-2',  name: '外科',  buzzerNumber: 2,  score: 0, color: '#ff6b6b' },
  { id: 'team-3',  name: '儿科',  buzzerNumber: 3,  score: 0, color: '#6bcb77' },
  { id: 'team-4',  name: '妇产科', buzzerNumber: 4,  score: 0, color: '#ff8fab' },
  { id: 'team-5',  name: '急诊科', buzzerNumber: 5,  score: 0, color: '#ffd93d' },
  { id: 'team-6',  name: '麻醉科', buzzerNumber: 6,  score: 0, color: '#c084fc' },
  { id: 'team-7',  name: '检验科', buzzerNumber: 7,  score: 0, color: '#fb923c' },
  { id: 'team-8',  name: '影像科', buzzerNumber: 8,  score: 0, color: '#34d399' },
  { id: 'team-9',  name: '药剂科', buzzerNumber: 9,  score: 0, color: '#f472b6' },
  { id: 'team-10', name: '护理部', buzzerNumber: 10, score: 0, color: '#38bdf8' },
];

// ==================== Question Bank ====================
let questions: Question[] = [
  { id: 1, text: '人体最大的器官是什么？', options: ['心脏', '肝脏', '皮肤', '大脑'] },
  { id: 2, text: '正常成人的静息心率范围是多少？（次/分钟）', options: ['40-60', '60-100', '100-120', '120-140'] },
  { id: 3, text: '以下哪种维生素可以通过阳光照射在皮肤中合成？', options: ['维生素A', '维生素B', '维生素C', '维生素D'] },
  { id: 4, text: '"白大褂"的发明最初是为了什么？', options: ['彰显权威', '便于清洁消毒', '区分科室', '保暖'] },
  { id: 5, text: '人体中含量最多的物质是什么？', options: ['蛋白质', '脂肪', '水', '钙'] },
  { id: 6, text: '世界卫生组织的缩写是什么？', options: ['WTO', 'WHO', 'WTF', 'WIPO'] },
  { id: 7, text: '"医者仁心"最早出自哪本古籍？', options: ['《黄帝内经》', '《本草纲目》', '《千金要方》', '《伤寒杂病论》'] },
  { id: 8, text: '护理人员"三查七对"中的"七对"不包括以下哪项？', options: ['床号', '姓名', '年龄', '药品'] },
  { id: 9, text: '医生在病历上写的"QD"是什么意思？', options: ['每天一次', '每四小时', '紧急', '停止'] },
  { id: 10, text: '以下哪个不是医院常见的科室？', options: ['内科', '外科', '天文科', '儿科'] },
  { id: 11, text: '"手术室"的无菌级别是？', options: ['Ⅰ级', 'Ⅱ级', 'Ⅲ级', 'Ⅳ级'] },
  { id: 12, text: '哪项检查被称为"医生的听诊器延伸"？', options: ['CT', 'MRI', '超声', '心电图'] },
  { id: 13, text: '医院里"120"这个号码通常代表什么？', options: ['急救电话', '保卫科', '总值班', '投诉热线'] },
  { id: 14, text: '医学上"生命体征"不包括以下哪项？', options: ['体温', '脉搏', '体重', '血压'] },
  { id: 15, text: '"希波克拉底誓言"是哪个职业的职业道德准则？', options: ['护士', '医生', '药师', '技师'] },
];
...
io.on('connection', (socket) => {
  ...
  socket.on('host:next-question', () => {
    ...
    state.currentQuestion = questions[state.questionIndex];
    resetForNewQuestion();
    state.mode = 'reading';
    broadcastState();
    console.log(`[host] → reading, question #${state.currentQuestion.id}`);
  });
  ...
  // ---- Host: enter buzzer winner manually (which buzzer 1-10 won) ----
  socket.on('host:buzzer-winner', (buzzerNumber: number) => {
    const team = state.teams.find(t => t.buzzerNumber === buzzerNumber);
    if (!team) {
    ...
    state.mode = 'buzzed';
    broadcastState();
    console.log(`[host] → buzzed, winner: ${team.name} (#${buzzerNumber})`);
  });
  ...
  // ---- Admin: import questions (batch) ----
  socket.on('admin:import-questions', (data: { lines: string }) => {
    ...
    for (const line of lines) {
      ...
      if (parts.length >= 5) {
        ...
        if (text && options.every((o: string) => o)) {
          imported.push({ id: nextQuestionId++, text, options });
          continue;
        }
      }
      // Try format: text with no options (just a display question)
      if (parts.length === 1 && parts[0]) {
        imported.push({ id: nextQuestionId++, text: parts[0] });
        continue;
      }
      errors++;
    }
